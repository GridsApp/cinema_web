...
class ManageBookings extends Component
{
    ...
    public function render()
    {
        ...
        $movie_shows = MovieShow::with([
            'time' => function($query) { $query->select('id', 'label'); },
            'theater' => function($query) { $query->select("id","label","branch_id","hall_number","price_group_id"); },
            'theater.branch' =>function($query) { $query->select('id', 'label_en' , 'label_ar'); },
            'theater.priceGroup' => function($query) { $query->select('id', 'label'); },
