        ...
        <div class="location">
            <div class="flex flex-col gap-1">
                ...
                @if (isset($branch['latitude']) && isset($branch['longitude']))
                    <button 
                        @click="showModal = true; console.log('Modal Opened')" 
                        class="primary-color text-[12px] underline"
                    >
                        View Location
                    </button>
                @endif
...
<div 
    x-cloak 
    x-show="showModal" 
    @click.outside="showModal = false; console.log('Modal Closed')" 
    x-transition
    class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50"
>
    <div class="bg-white rounded-lg p-[44px] max-w-[500px]" @click.stop>
        <button 
            @click="showModal = false; console.log('Modal Closed')" 
            class="pb-5 text-center flex justify-end w-full text-black rounded"
        >
            <i class="fa-solid fa-x"></i>
        </button>
        <h2 class="text-[12px] mb-4">
            Do you like to continue and see the movies in this branch?
        </h2>
        <div class="w-full flex justify-center">
            <div class="flex flex-col gap-4 w-[80%] justify-center">
                <div>
                    <a 
                        :href="`/${cinemaPrefix}/${languagePrefix}/movies/listing`"
                ...
                <div>
                    <a 
                        ...
                        target="_blank"
                        class="primary-color flex justify-center flex-col items-center text-[14px] underline"
                    >
                        View Location
                    </a>
                </div>
